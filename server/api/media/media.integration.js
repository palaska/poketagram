...
describe('Media API:', function() {
  ...
  describe('GET /api/medias', function() {
    var medias;
  ...
  describe('POST /api/medias', function() {
    beforeEach(function(done) {
      request(app)
        .post('/api/medias')
        .send({
  ...
  describe('GET /api/medias/:id', function() {
    var media;

    beforeEach(function(done) {
      request(app)
        .get('/api/medias/' + newMedia._id)
        .expect(200)
        .expect('Content-Type', /json/)
        .end(function(err, res) {
          if (err) {
  ...
  describe('PUT /api/medias/:id', function() {
    var updatedMedia

    beforeEach(function(done) {
      request(app)
        .put('/api/medias/' + newMedia._id)
        .send({
          name: 'Updated Media',
  ...
  describe('DELETE /api/medias/:id', function() {

    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete('/api/medias/' + newMedia._id)
        .expect(204)
        .end(function(err, res) {
          if (err) {
            return done(err);
